...
std::string deviceName(cl::sycl::info::device_type type) {
  //@formatter:off
  switch (type) {
...
void printDetailed(const cl::sycl::device& device, size_t index) {
  ...
  std::cout << "      - Profile : "
            << platform.get_info<cl::sycl::info::platform::profile>() << "\n";
}
...
void printHelp(const std::string& name) {
  ...
  std::cout
      ...
      << "      --device <INDEX>     Select device at INDEX from output of "
         "--list\n"
      << "      --file               Custom clover.in file (defaults to "
         "clover.in if unspecified)\n"
      << std::endl;
}
...
RunConfig parseArgs(const std::vector<cl::sycl::device>& devices,
  ...
  const auto readParam = [&args](size_t current,
                                 const std::string& emptyMessage, auto map) {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
      ...
      readParam(i, "--device specified but no size was given",
                [&config](const auto& param) {
                  try {
                    config.device =
                        cl::sycl::device::get_devices().at(std::stoul(param));
                  } catch (const std::exception& e) {
                    std::cerr << "failed to parse/select device index `"
                              << param << "`:" << e.what() << std::endl;
                    std::exit(EXIT_FAILURE);
                  }
                });
    } else if (arg == "--file") {
      readParam(i, "--file specified but no file was given",
                [&config](const auto& param) { config.file = param; });
    }
  }
  return config;
}

std::unique_ptr<global_variables> initialise(
    parallel_& parallel, const std::vector<std::string>& args) {
  global_config config;

  if (parallel.boss) {
    of.open("clover.out");
    if (!of.is_open())
      report_error((char*) "initialise",
                   (char*) "Error opening clover.out file.");
  ...
  if (parallel.boss) {
    ...
    std::cout << "Output file clover.out opened. All output will go there."
              << std::endl;
  }
  ...
  std::cout << "Using SYCL device: "
            ...
            << deviceName(selectedDevice
                              .get_info<cl::sycl::info::device::device_type>())
            << ")" << std::endl;
  ...
  if (parallel.boss) {
    ...
    if (!g_in.good()) {
      std::cerr << "Unable to open file: `" << file << "`, using defaults"
                << std::endl;
      g_in.close();
