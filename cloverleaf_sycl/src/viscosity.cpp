...
void viscosity_kernel(handler& h, int x_min, int x_max, int y_min, int y_max,
  ...
  // DO k=y_min,y_max
  //   DO j=x_min,x_max
  clover::par_ranged<class viscosity_>(
      h, {x_min + 1, y_min + 1, x_max + 2, y_max + 2}, [=](id<2> idx) {
        double ugrad = (xvel0[clover::offset(idx, 1, 0)] +
                        xvel0[clover::offset(idx, 1, 1)]) -
                       (xvel0[idx] + xvel0[clover::offset(idx, 0, 1)]);

        double vgrad = (yvel0[clover::offset(idx, 0, 1)] +
                        yvel0[clover::offset(idx, 1, 1)]) -
                       (yvel0[idx] + yvel0[clover::offset(idx, 1, 0)]);

        double div = (celldx[idx[0]] * (ugrad) + celldy[idx[1]] * (vgrad));

        double strain2 = 0.5 *
                             (xvel0[clover::offset(idx, 0, 1)] +
                              xvel0[clover::offset(idx, 1, 1)] - xvel0[idx] -
                              xvel0[clover::offset(idx, 1, 0)]) /
                             celldy[idx[1]] +
                         0.5 *
                             (yvel0[clover::offset(idx, 1, 0)] +
                              yvel0[clover::offset(idx, 1, 1)] - yvel0[idx] -
                              yvel0[clover::offset(idx, 0, 1)]) /
                             celldx[idx[0]];

        double pgradx = (pressure[clover::offset(idx, 1, 0)] -
                         pressure[clover::offset(idx, -1, 0)]) /
                        (celldx[idx[0]] + celldx[idx[0] + 1]);
        double pgrady = (pressure[clover::offset(idx, 0, 1)] -
                         pressure[clover::offset(idx, 0, -1)]) /
                        (celldy[idx[1]] + celldy[idx[1] + 2]);

        double pgradx2 = pgradx * pgradx;
        double pgrady2 = pgrady * pgrady;

        double limiter = ((0.5 * (ugrad) / celldx[idx[0]]) * pgradx2 +
                          (0.5 * (vgrad) / celldy[idx[1]]) * pgrady2 +
                          strain2 * pgradx * pgrady) /
                         cl::sycl::fmax(pgradx2 + pgrady2, 1.0e-16);
        ...
        if ((limiter > 0.0) || (div >= 0.0)) {
          ...
          if (pgradx < 0.0)
            dirx = -1.0;
          pgradx = dirx * cl::sycl::fmax(1.0e-16, cl::sycl::fabs(pgradx));
          double diry = 1.0;
          if (pgradx < 0.0)
            diry = -1.0;
          pgrady = diry * cl::sycl::fmax(1.0e-16, cl::sycl::fabs(pgrady));
          double pgrad = cl::sycl::sqrt(pgradx * pgradx + pgrady * pgrady);
          double xgrad = cl::sycl::fabs(celldx[idx[0]] * pgrad / pgradx);
          double ygrad = cl::sycl::fabs(celldy[idx[1]] * pgrad / pgrady);
          double grad = cl::sycl::fmin(xgrad, ygrad);
...
void viscosity(global_variables& globals) {
  for (int tile = 0; tile < globals.config.tiles_per_chunk; ++tile) {
    ...
    clover::execute(globals.queue, [&](handler& h) {
      viscosity_kernel(
          h, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin, t.info.t_ymax,
          t.field.celldx.access<R>(h), t.field.celldy.access<R>(h),
          t.field.density0.access<R>(h), t.field.pressure.access<R>(h),
          t.field.viscosity.access<W>(h), t.field.xvel0.access<R>(h),
