...
void advec_mom_kernel(queue& q, int x_min, int x_max, int y_min, int y_max,
                      ...
                      clover::Buffer<double, 1>& celldy_buffer, int which_vel,
  ...
  clover::Range2d policy(x_min - 2 + 1, y_min - 2 + 1, x_max + 2 + 2,
                         y_max + 2 + 2);
  ...
  if (mom_sweep == 1) {  // x 1
      ...
      clover::par_ranged<class APPEND_LN(advec_mom_x1)>(
          h, policy, [=](id<2> idx) {
            post_vol[idx] = volume[idx] +
                            vol_flux_y[clover::offset(idx, 0, 1)] -
                            vol_flux_y[idx];
            pre_vol[idx] = post_vol[idx] +
                           vol_flux_x[clover::offset(idx, 1, 0)] -
                           vol_flux_x[idx];
          });
    });
    ...
    clover::execute(q, [&](handler& h) {
      ...
      auto pre_vol = pre_vol_buffer.access<W>(h);
      auto post_vol = post_vol_buffer.access<RW>(h);
      clover::par_ranged<class APPEND_LN(advec_mom_y1)>(
          h, policy, [=](id<2> idx) {
            post_vol[idx] = volume[idx] +
                            vol_flux_x[clover::offset(idx, 1, 0)] -
                            vol_flux_x[idx];
  ...
  if (direction == 1) {
    if (which_vel == 1) {
      ...
      clover::execute(q, [&](handler& h) {
        ...
        clover::par_ranged<class advec_mom_dir1_vel1_node_flux>(
            h, {x_min - 2 + 1, y_min + 1, x_max + 2 + 2, y_max + 1 + 2},
            [=](id<2> idx) {
              // Find staggered mesh mass fluxes, nodal masses and volumes.
              node_flux[idx] = 0.25 * (mass_flux_x[clover::offset(idx, 0, -1)] +
                                       mass_flux_x[idx] +
      ...
      clover::execute(q, [&](handler& h) {
        ...
        clover::par_ranged<class advec_mom_dir1_vel1_node_mass_pre>(
            h, {x_min - 1 + 1, y_min + 1, x_max + 2 + 2, y_max + 1 + 2},
            [=](id<2> idx) {
              // Staggered cell mass post advection
              node_mass_post[idx] =
                  0.25 * (density1[clover::offset(idx, 0, -1)] *
                              post_vol[clover::offset(idx, 0, -1)] +
                          density1[idx] * post_vol[idx] +
                          density1[clover::offset(idx, -1, -1)] *
                              post_vol[clover::offset(idx, -1, -1)] +
                          density1[clover::offset(idx, -1, 0)] *
                              post_vol[clover::offset(idx, -1, 0)]);
              node_mass_pre[idx] = node_mass_post[idx] -
                                   node_flux[clover::offset(idx, -1, 0)] +
                                   node_flux[idx];
            });
      });
    }
    ...
    clover::execute(q, [&](handler& h) {
      ...
      clover::par_ranged<class advec_mom_dir1_mom_flux>(
          h, {x_min - 1 + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2},
          [=](id<2> idx) {
            int upwind, donor, downwind, dif;
            double sigma, width, limiter, vdiffuw, vdiffdw, auw, adw, wind,
                advec_vel_s;
            ...
            sigma = cl::sycl::fabs(node_flux[idx]) / (node_mass_pre[donor][k]);
            width = celldx[j];
            vdiffuw = vel1[donor][k] - vel1[upwind][k];
            vdiffdw = vel1[downwind][k] - vel1[donor][k];
            limiter = 0.0;
            if (vdiffuw * vdiffdw > 0.0) {
              auw = cl::sycl::fabs(vdiffuw);
              adw = cl::sycl::fabs(vdiffdw);
              wind = 1.0;
              if (vdiffdw <= 0.0)
                wind = -1.0;
              limiter = wind * cl::sycl::fmin(
                                   cl::sycl::fmin(
                                       width *
                                           ((2.0 - sigma) * adw / width +
                                            (1.0 + sigma) * auw / celldx[dif]) /
                                           6.0,
                                       auw),
                                   adw);
            }
    ...
    clover::execute(q, [&](handler& h) {
      ...
      clover::par_ranged<class advec_mom_dir1_vel1>(
          h, {x_min + 1, y_min + 1, x_max + 1 + 2, y_max + 1 + 2},
          [=](id<2> idx) {
            vel1[idx] = (vel1[idx] * node_mass_pre[idx] +
                         mom_flux[clover::offset(idx, -1, 0)] - mom_flux[idx]) /
                        node_mass_post[idx];
          });
    });
  } else if (direction == 2) {
    if (which_vel == 1) {
      ...
      clover::execute(q, [&](handler& h) {
        ...
        clover::par_ranged<class advec_mom_dir2_vel1_node_flux>(
            h, {x_min + 1, y_min - 2 + 1, x_max + 1 + 2, y_max + 2 + 2},
            [=](id<2> idx) {
              // Find staggered mesh mass fluxes and nodal masses and volumes.
              node_flux[idx] = 0.25 * (mass_flux_y[clover::offset(idx, -1, 0)] +
                                       mass_flux_y[idx] +
      ...
      clover::execute(q, [&](handler& h) {
        ...
        clover::par_ranged<class advec_mom_dir2_vel1_node_mass_pre>(
            h, {x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 2 + 2},
            [=](id<2> idx) {
              node_mass_post[idx] =
                  0.25 * (density1[clover::offset(idx, 0, -1)] *
                              post_vol[clover::offset(idx, 0, -1)] +
                          ...
                          density1[clover::offset(idx, -1, 0)] *
                              post_vol[clover::offset(idx, -1, 0)]);
              node_mass_pre[idx] = node_mass_post[idx] -
                                   node_flux[clover::offset(idx, 0, -1)] +
                                   node_flux[idx];
            });
      });
    }
    ...
    clover::execute(q, [&](handler& h) {
      ...
      clover::par_ranged<class advec_mom_dir2_mom_flux>(
          h, {x_min + 1, y_min - 1 + 1, x_max + 1 + 2, y_max + 1 + 2},
          [=](id<2> idx) {
            int upwind, donor, downwind, dif;
            double sigma, width, limiter, vdiffuw, vdiffdw, auw, adw, wind,
                advec_vel_s;
            ...
            sigma = cl::sycl::fabs(node_flux[idx]) / (node_mass_pre[j][donor]);
            width = celldy[k];
    ...
    clover::execute(q, [&](handler& h) {
      ...
      clover::par_ranged<class advec_mom_dir2_vel1>(
          ...
          [=](id<2> idx) {
            vel1[idx] = (vel1[idx] * node_mass_pre[idx] +
                         mom_flux[clover::offset(idx, 0, -1)] - mom_flux[idx]) /
                        node_mass_post[idx];
          });
    });
  }
...
void advec_mom_driver(global_variables& globals, int tile, int which_vel,
                      int direction, int sweep_number) {
  tile_type& t = globals.chunk.tiles[tile];
  if (which_vel == 1) {
    advec_mom_kernel(globals.queue, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin,
                     t.info.t_ymax, t.field.xvel1, t.field.mass_flux_x,
                     t.field.vol_flux_x, t.field.mass_flux_y,
                     t.field.vol_flux_y, t.field.volume, t.field.density1,
                     t.field.work_array1, t.field.work_array2,
                     t.field.work_array3, t.field.work_array4,
                     t.field.work_array5, t.field.work_array6, t.field.celldx,
                     t.field.celldy, which_vel, sweep_number, direction);
  } else {
    advec_mom_kernel(globals.queue, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin,
                     t.info.t_ymax, t.field.yvel1, t.field.mass_flux_x,
                     t.field.vol_flux_x, t.field.mass_flux_y,
                     t.field.vol_flux_y, t.field.volume, t.field.density1,
                     t.field.work_array1, t.field.work_array2,
                     t.field.work_array3, t.field.work_array4,
