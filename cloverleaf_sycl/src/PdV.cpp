...
void PdV_kernel(handler& h, bool predict, int x_min, int x_max, int y_min,
                int y_max, double dt,
  ...
  if (predict) {
    clover::par_ranged<class PdV_predict_true>(h, policy, [=](id<2> idx) {
      double left_flux =
          (xarea[idx] * (xvel0[idx] + xvel0[clover::offset(idx, 0, 1)] +
                         xvel0[idx] + xvel0[clover::offset(idx, 0, 1)])) *
          0.25 * dt * 0.5;

      double right_flux = (xarea[clover::offset(idx, 1, 0)] *
      ...
      double bottom_flux =
          (yarea[idx] * (yvel0[idx] + yvel0[clover::offset(idx, 1, 0)] +
                         yvel0[idx] + yvel0[clover::offset(idx, 1, 0)])) *
          0.25 * dt * 0.5;

      double top_flux = (yarea[clover::offset(idx, 0, 1)] *
      ...
      double min_cell_volume =
          cl::sycl::fmin(cl::sycl::fmin(volume[idx] + right_flux - left_flux +
    ...
    clover::par_ranged<class PdV_predict_false>(h, policy, [=](id<2> idx) {
      double left_flux =
          (xarea[idx] * (xvel0[idx] + xvel0[clover::offset(idx, 0, 1)] +
                         xvel1[idx] + xvel1[clover::offset(idx, 0, 1)])) *
          0.25 * dt;

      double right_flux = (xarea[clover::offset(idx, 1, 0)] *
                           (xvel0[clover::offset(idx, 1, 0)] +
                            xvel0[clover::offset(idx, 1, 1)] +
                            xvel1[clover::offset(idx, 1, 0)] +
                            xvel1[clover::offset(idx, 1, 1)])) *
                          0.25 * dt;

      double bottom_flux =
          (yarea[idx] * (yvel0[idx] + yvel0[clover::offset(idx, 1, 0)] +
                         yvel1[idx] + yvel1[clover::offset(idx, 1, 0)])) *
          0.25 * dt;

      double top_flux = (yarea[clover::offset(idx, 0, 1)] *
                         (yvel0[clover::offset(idx, 0, 1)] +
                          yvel0[clover::offset(idx, 1, 1)] +
                          yvel1[clover::offset(idx, 0, 1)] +
                          yvel1[clover::offset(idx, 1, 1)])) *
                        0.25 * dt;
...
void PdV(global_variables& globals, bool predict) {
  ...
  clover::execute(globals.queue, [&](handler& h) {
    for (int tile = 0; tile < globals.config.tiles_per_chunk; ++tile) {
      ...
      PdV_kernel(h, predict, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin,
                 t.info.t_ymax, globals.dt, t.field.xarea.access<RW>(h),
                 t.field.yarea.access<RW>(h), t.field.volume.access<RW>(h),
                 t.field.density0.access<RW>(h), t.field.density1.access<RW>(h),
                 t.field.energy0.access<RW>(h), t.field.energy1.access<RW>(h),
                 t.field.pressure.access<RW>(h),
                 t.field.viscosity.access<RW>(h), t.field.xvel0.access<RW>(h),
                 t.field.xvel1.access<RW>(h), t.field.yvel0.access<RW>(h),
