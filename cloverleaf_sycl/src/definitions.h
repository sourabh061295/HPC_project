...
#define APPEND_LN(base) \
  PP_CAT(base, __LINE__)  // appends line number to the given base name
...
static inline void record(const std::string& name,
                          const std::function<void(std::ofstream&)>& f) {
  std::ofstream out;
...
static inline void show(std::ostream& out, const std::string& name,
                        clover::Buffer<double, 1>& buffer) {
...
// In the Fortran version these are 1,2,3,4,-1, but they are used firectly to
// index an array in this version
...
struct field_type {
  ...
  clover::Buffer<double, 2>
      work_array1;  // node_flux, stepbymass, volume_change, pre_vol
...
struct chunk_type {
  ...
  // MPI Buffers in host memory - to be created with
  // Kokkos::create_mirror_view() and Kokkos::deep_copy()
  //	std::vector<double > hm_left_rcv_buffer, hm_right_rcv_buffer,
  // hm_bottom_rcv_buffer, hm_top_rcv_buffer; 	std::vector<double >
  // hm_left_snd_buffer, hm_right_snd_buffer, hm_bottom_snd_buffer,
  // hm_top_snd_buffer;
  const std::array<int, 4>
      chunk_neighbours;  // Chunks, not tasks, so we can overload in the future
  ...
  clover::Buffer<double, 1> left_rcv_buffer, right_rcv_buffer,
      bottom_rcv_buffer, top_rcv_buffer;
  clover::Buffer<double, 1> left_snd_buffer, right_snd_buffer,
      bottom_snd_buffer, top_snd_buffer;

  std::vector<tile_type> tiles;

  chunk_type(const std::array<int, 4>& chunkNeighbours, const int task,
             const int xMin, const int yMin, const int xMax, const int yMax,
             const int left, const int right, const int bottom, const int top,
             const int leftBoundary, const int rightBoundary,
...
struct global_variables {
  ...
  bool profiler_on;  // Internal code profiler to make comparisons accross
                     // systems easier

  profiler_type profiler;

  explicit global_variables(const global_config& config, cl::sycl::queue queue,
                            chunk_type chunk)
      : config(config),
      ...
      for (size_t i = 0; i < chunk.tiles.size(); ++i) {
        ...
        for (int l = 0; l < 4; ++l) {
          out << "info.tile_neighbours[i]" << '=' << info.tile_neighbours[i]
              << std::endl;
          out << "info.external_tile_mask[i]" << '='
              << info.external_tile_mask[i] << std::endl;
        }
