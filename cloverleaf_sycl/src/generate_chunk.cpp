...
void generate_chunk(const int tile, global_variables& globals) {
  ...
  clover::Buffer<double, 1> hm_state_density(state_density_vec.begin(),
                                             state_density_vec.end());
  clover::Buffer<double, 1> hm_state_energy(state_energy_vec.begin(),
                                            state_energy_vec.end());
  clover::Buffer<double, 1> hm_state_xvel(state_xvel_vec.begin(),
                                          state_xvel_vec.end());
  clover::Buffer<double, 1> hm_state_yvel(state_yvel_vec.begin(),
                                          state_yvel_vec.end());
  clover::Buffer<double, 1> hm_state_xmin(state_xmin_vec.begin(),
                                          state_xmin_vec.end());
  clover::Buffer<double, 1> hm_state_xmax(state_xmax_vec.begin(),
                                          state_xmax_vec.end());
  clover::Buffer<double, 1> hm_state_ymin(state_ymin_vec.begin(),
                                          state_ymin_vec.end());
  clover::Buffer<double, 1> hm_state_ymax(state_ymax_vec.begin(),
                                          state_ymax_vec.end());
  clover::Buffer<double, 1> hm_state_radius(state_radius_vec.begin(),
                                            state_radius_vec.end());
  clover::Buffer<int, 1> hm_state_geometry(state_geometry_vec.begin(),
                                           state_geometry_vec.end());
  ...
  // Take a reference to the lowest structure, as Kokkos device cannot
  // necessarily chase through the structure.
  ...
  clover::execute(globals.queue, [&](handler& h) {
    ...
    clover::par_ranged<class generate_chunk_1>(
        h, xyrange_policy, [=](id<2> idx) {
  ...
  for (int state = 1; state < globals.config.number_of_states; ++state) {
    clover::execute(globals.queue, [&](handler& h) {
      ...
      clover::par_ranged<class generate_chunk_2>(
          h, xyrange_policy, [=](id<2> idx) {
            ...
            if (state_geometry[state] == g_rect) {
              ...
              double radius =
                  cl::sycl::sqrt((cellx[j] - x_cent) * (cellx[j] - x_cent) +
