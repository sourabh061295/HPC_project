...
struct local_reducer {
  ...
  C actual;
  cl::sycl::accessor<U, 1, cl::sycl::access::mode::read_write,
                     cl::sycl::access::target::global_buffer>
      result;

  local_reducer(cl::sycl::handler& h, size_t size, C actual,
                cl::sycl::buffer<U, 1>& b)
      : local(cl::sycl::range<1>(size), h),
        actual(actual),
        result(b.template get_access<cl::sycl::access::mode::read_write>(h)) {}

  inline void drain(cl::sycl::id<1> lid, cl::sycl::id<1> gid) const {
    local[lid] = result[gid];
  }
};

template <typename nameT, size_t dimension, class RangeTpe, class LocalType,
          class LocalAllocator = std::nullptr_t, class Empty = std::nullptr_t,
          class Functor = std::nullptr_t, class BinaryOp = std::nullptr_t,
          class Finaliser = std::nullptr_t,
          class RangeLengthFn = std::nullptr_t,
          class RangeIdFn = std::nullptr_t>
static inline void par_reduce_nd_impl(cl::sycl::queue& q, RangeTpe range,
                                      RangeLengthFn lengthFn,
                                      RangeIdFn rangeIdFn,
                                      LocalAllocator allocator, Empty empty,
                                      Functor functor, BinaryOp combiner,
                                      Finaliser finaliser) {
  ...
  if (dev.is_cpu()) {
    ...
    dot_wgsize =
        dev.get_info<cl::sycl::info::device::native_vector_width_double>() * 2;

  } else {
    dot_num_groups =
        dev.get_info<cl::sycl::info::device::max_compute_units>() * 4;
...
#ifdef SYCL_DEBUG
  std::cout << "RD: dot_wgsize=" << dot_wgsize
            << " dot_num_groups:" << dot_num_groups << " N=" << N << "\n";
#endif
    ...
    h.parallel_for<nameT>(
        ...
        [=](cl::sycl::nd_item<1> item) {
          ...
          for (; i < N; i += global_size) {
            functor(ctx, cl::sycl::id<1>(li),
                    rangeIdFn(cl::sycl::id<1>(i), range));
          }
    ...
    h.parallel_for<class final_reduction>(
        cl::sycl::nd_range<1>(1, 1), [=](auto) {
          ...
          for (size_t i = 1; i < dot_num_groups; ++i) {
            combiner(ctx, zero,
                     cl::sycl::id<1>(i));  // local[0] = local[0] |+| xs[i]
          }
...
template <typename nameT, class LocalType,
          class LocalAllocator = std::nullptr_t, class Empty = std::nullptr_t,
          class Functor = std::nullptr_t, class BinaryOp = std::nullptr_t,
          class Finaliser = std::nullptr_t>
static inline void par_reduce_2d(cl::sycl::queue& q,
                                 const clover::Range2d& range,
                                 LocalAllocator allocator, Empty empty,
                                 Functor functor, BinaryOp combiner,
                                 Finaliser finaliser) {
#ifdef SYCL_DEBUG
...
#endif
  par_reduce_nd_impl<nameT, 2, clover::Range2d, LocalType, LocalAllocator,
                     Empty, Functor, BinaryOp, Finaliser>(
...
template <typename nameT, class LocalType,
          ...
          class Functor = std::nullptr_t, class BinaryOp = std::nullptr_t,
          class Finaliser = std::nullptr_t>
static inline void par_reduce_1d(cl::sycl::queue& q,
                                 const clover::Range1d& range,
                                 LocalAllocator allocator, Empty empty,
                                 Functor functor, BinaryOp combiner,
                                 Finaliser finaliser) {
#ifdef SYCL_DEBUG
...
#endif
  par_reduce_nd_impl<nameT, 1, clover::Range1d, LocalType, LocalAllocator,
                     Empty, Functor, BinaryOp, Finaliser>(
