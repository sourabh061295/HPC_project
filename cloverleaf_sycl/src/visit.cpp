...
void visit(global_variables& globals, parallel_& parallel) {
  ...
  if (parallel.boss) {
    if (first_call) {
      int nblocks =
          globals.config.number_of_chunks * globals.config.tiles_per_chunk;
  ...
  for (int tile = 0; tile < globals.config.tiles_per_chunk; ++tile) {
    if (globals.chunk.task == parallel.task) {
      int nxc = globals.chunk.tiles[tile].info.t_xmax -
                globals.chunk.tiles[tile].info.t_xmin + 1;
      int nyc = globals.chunk.tiles[tile].info.t_ymax -
                globals.chunk.tiles[tile].info.t_ymin + 1;
      int nxv = nxc + 1;
      int nyv = nyc + 1;
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
          ...
          u << std::scientific << std::setprecision(3) << hm_density0[j][k]
            << std::endl;
        }
      }
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
             j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
          u << std::scientific << std::setprecision(3) << hm_energy0[j][k]
            << std::endl;
        }
      }
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
             j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
          u << std::scientific << std::setprecision(3) << hm_pressure[j][k]
            << std::endl;
        }
      }
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
             j <= globals.chunk.tiles[tile].info.t_xmax + 1; ++j) {
          double temp = (cl::sycl::fabs(hm_viscosity[j][k]) > 0.00000001)
                            ? hm_viscosity[j][k]
                            : 0.0;
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
             j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
          double temp = (cl::sycl::fabs(hm_xvel0[j][k]) > 0.00000001)
                            ? hm_xvel0[j][k]
                            : 0.0;
          u << std::scientific << std::setprecision(3) << temp << std::endl;
        }
      ...
      for (int k = globals.chunk.tiles[tile].info.t_ymin + 1;
        ...
        for (int j = globals.chunk.tiles[tile].info.t_xmin + 1;
             j <= globals.chunk.tiles[tile].info.t_xmax + 1 + 1; ++j) {
          double temp = (cl::sycl::fabs(hm_yvel0[j][k]) > 0.00000001)
                            ? hm_yvel0[j][k]
                            : 0.0;
          u << std::scientific << std::setprecision(3) << temp << std::endl;
        }
