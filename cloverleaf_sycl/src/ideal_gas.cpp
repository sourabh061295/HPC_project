...
void ideal_gas_kernel(handler& h, int x_min, int x_max, int y_min, int y_max,
  ...
  // std::cout <<" ideal_gas(" << x_min+1 << ","<< y_min+1<< ","<< x_max+2<<
  // ","<< y_max +2  << ")" << std::endl;
  //  DO k=y_min,y_max
  //    DO j=x_min,x_max
  ...
  clover::par_ranged<class ideal_gas>(
      h, {x_min + 1, y_min + 1, x_max + 2, y_max + 2}, [=](id<2> idx) {
        ...
        double sound_speed_squared =
            v * v * (pressure[idx] * pressurebyenergy - pressurebyvolume);
        soundspeed[idx] = cl::sycl::sqrt(sound_speed_squared);
      });
}
...
void ideal_gas(global_variables& globals, const int tile, bool predict) {
  if (DEBUG)
    std::cout << "ideal_gas(tile " << tile << ")" << std::endl;
  ...
  clover::execute(globals.queue, [&](handler& h) {
    if (!predict) {
      ideal_gas_kernel(
          ...
          t.field.density0.access<R>(h), t.field.energy0.access<R>(h),
          t.field.pressure.access<RW>(h), t.field.soundspeed.access<RW>(h));
    } else {
      ideal_gas_kernel(
          h, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin, t.info.t_ymax,
          t.field.density1.access<R>(h), t.field.energy1.access<R>(h),
          t.field.pressure.access<RW>(h), t.field.soundspeed.access<RW>(h));
    }
  });
}
