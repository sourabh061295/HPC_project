...
void calc_dt_kernel(
    queue& q, int x_min, int x_max, int y_min, int y_max, double dtmin,
    ...
    clover::Buffer<double, 1> cellx, clover::Buffer<double, 1> celly,
    ...
    double& dt_min_val, int& dtl_control, double& xl_pos, double& yl_pos,
    int& jldt, int& kldt, int& small) {
  small = 0;
  dt_min_val = g_big;
  ...
  clover::execute(q, [&](handler& h) {
    ...
    clover::par_ranged<class APPEND_LN(advec_mom_x1)>(
        h, policy, [=](id<2> idx) {
          ...
          cc = cl::sycl::fmax(cl::sycl::sqrt(cc), g_small);

          double dtct = dtc_safe * cl::sycl::fmin(dsx, dsy) / cc;

          double div = 0.0;

          double dv1 =
              (xvel0_[idx] + xvel0_[clover::offset(idx, 0, 1)]) * xarea_[idx];
          double dv2 = (xvel0_[clover::offset(idx, 1, 0)] +
                        xvel0_[clover::offset(idx, 1, 1)]) *
                       xarea_[clover::offset(idx, 1, 0)];

          div = div + dv2 - dv1;

          double dtut = dtu_safe * 2.0 * volume_[idx] /
                        cl::sycl::fmax(cl::sycl::fmax(cl::sycl::fabs(dv1),
                                                      cl::sycl::fabs(dv2)),
                                       g_small * volume_[idx]);

          dv1 = (yvel0_[idx] + yvel0_[clover::offset(idx, 1, 0)]) * yarea_[idx];
          dv2 = (yvel0_[clover::offset(idx, 0, 1)] +
                 yvel0_[clover::offset(idx, 1, 1)]) *
          ...
          double mins = cl::sycl::fmin(
              dtct,
              cl::sycl::fmin(
                  dtut, cl::sycl::fmin(dtvt, cl::sycl::fmin(dtdivt, g_big))));

          size_t idx1d =
              (idx[0] - policy.fromX) * policy.sizeY + (idx[1] - policy.fromY);
  ...
  clover::par_reduce_1d<class dt_kernel_reduce, double>(
      ...
      [](const ctx& ctx, size_t group, id<1> idx) {
        ctx.result[group] = ctx.local[idx];
      });
  ...
  clover::par_reduce_2d<class dt_kernel_reduce, double>(
      ...
      [=](handler& h, size_t& size) mutable {
        return ctx(
            h, size,
            {xarea.access<R>(h), yarea.access<R>(h), celldx.access<R>(h),
             celldy.access<R>(h), volume.access<R>(h), density0.access<R>(h),
             viscosity_a.access<R>(h), soundspeed.access<R>(h),
             xvel0.access<R>(h), yvel0.access<R>(h)},
            result.buffer);
      },
      [](const ctx& ctx, id<1> lidx) { ctx.local[lidx] = g_big; },
      [dtc_safe, dtv_safe, dtu_safe, dtdiv_safe](ctx ctx, id<1> lidx,
                                                 id<2> idx) {
        ...
        double dv1 = (ctx.actual.xvel0[idx] +
                      ctx.actual.xvel0[clover::offset(idx, 0, 1)]) *
                     ctx.actual.xarea[idx];
        double dv2 = (ctx.actual.xvel0[clover::offset(idx, 1, 0)] +
                      ctx.actual.xvel0[clover::offset(idx, 1, 1)]) *
        ...
        double dtut = dtu_safe * 2.0 * ctx.actual.volume[idx] /
                      cl::sycl::fmax(cl::sycl::fmax(cl::sycl::fabs(dv1),
                                                    cl::sycl::fabs(dv2)),
                                     g_small * ctx.actual.volume[idx]);

        dv1 = (ctx.actual.yvel0[idx] +
               ctx.actual.yvel0[clover::offset(idx, 1, 0)]) *
              ctx.actual.yarea[idx];
        dv2 = (ctx.actual.yvel0[clover::offset(idx, 0, 1)] +
               ctx.actual.yvel0[clover::offset(idx, 1, 1)]) *
        ...
        double mins = cl::sycl::fmin(
            ...
            cl::sycl::fmin(
                dtut, cl::sycl::fmin(dtvt, cl::sycl::fmin(dtdivt, g_big))));
        ctx.local[lidx] = cl::sycl::fmin(ctx.local[lidx], mins);
      },
      [](const ctx& ctx, id<1> idx, id<1> idy) {
        ctx.local[idx] = cl::sycl::fmin(ctx.local[idx], ctx.local[idy]);
      },
  ...
  dtl_control =
      static_cast<int>(10.01 * (jk_control - static_cast<int>(jk_control)));
  jk_control = jk_control - (jk_control - (int) (jk_control));
  jldt = ((int) jk_control) % x_max;
  kldt = static_cast<int>(1.f + (jk_control / x_max));
  // TODO: cannot do this with GPU memory directly
  // xl_pos = cellx(jldt+1); // Offset by 1 because of Fortran halos in original
  // code yl_pos = celly(kldt+1);
  ...
  if (small != 0) {
    ...
    std::cout << "Timestep information:" << std::endl
              ...
              << "x, y                 : " << cellx_acc[jldt] << " "
              << celly_acc[kldt] << std::endl
              << "timestep : " << dt_min_val << std::endl
              << "Cell velocities;" << std::endl
              << xvel0_acc[jldt][kldt] << " " << yvel0_acc[jldt][kldt]
              << std::endl
              << xvel0_acc[jldt + 1][kldt] << " " << yvel0_acc[jldt + 1][kldt]
              << std::endl
              << xvel0_acc[jldt + 1][kldt + 1] << " "
              << yvel0_acc[jldt + 1][kldt + 1] << std::endl
              << xvel0_acc[jldt][kldt + 1] << " " << yvel0_acc[jldt][kldt + 1]
              << std::endl
...
void calc_dt(global_variables& globals, int tile, double& local_dt,
             std::string& local_control, double& xl_pos, double& yl_pos,
             int& jldt, int& kldt) {
  local_dt = g_big;
  ...
  calc_dt_kernel(
      globals.queue, t.info.t_xmin, t.info.t_xmax, t.info.t_ymin, t.info.t_ymax,
      globals.config.dtmin, globals.config.dtc_safe, globals.config.dtu_safe,
      globals.config.dtv_safe, globals.config.dtdiv_safe, t.field.xarea,
      t.field.yarea, t.field.cellx, t.field.celly, t.field.celldx,
      t.field.celldy, t.field.volume, t.field.density0, t.field.energy0,
      t.field.pressure, t.field.viscosity, t.field.soundspeed, t.field.xvel0,
      t.field.yvel0, local_dt, l_control, xl_pos, yl_pos, jldt, kldt, small);
