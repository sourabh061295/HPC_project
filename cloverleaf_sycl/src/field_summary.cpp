...
//  @author Wayne Gaudin
//  @details The user specified field summary kernel is invoked here. A
//  summation across all mesh chunks is then performed and the information
//  outputed. If the run is a test problem, the final result is compared with
//  the expected result and the difference output. Note the reference solution
//  is the value returned from an Intel compiler with ieee options set on a
//  single core crun.
...
void field_summary(global_variables& globals, parallel_& parallel) {
  ...
  for (int tile = 0; tile < globals.config.tiles_per_chunk; ++tile) {
    ...
    clover::par_reduce_1d<class field_summary, value_type>(
        ...
        [=](handler& h, size_t& size) mutable {
          return ctx(
              h, size,
              {t.field.volume.access<R>(h), t.field.density0.access<R>(h),
               t.field.energy0.access<R>(h), t.field.pressure.access<R>(h),
               t.field.xvel0.access<R>(h), t.field.yvel0.access<R>(h)},
              result.buffer);
        },
        ...
        [ymin, xmax, xmin](const ctx& ctx, id<1> lidx, id<1> idx) {
          ...
          for (size_t kv = k; kv <= k + 1; ++kv) {
            for (size_t jv = j; jv <= j + 1; ++jv) {
              vsqrd +=
                  0.25 * (ctx.actual.xvel0[jv][kv] * ctx.actual.xvel0[jv][kv] +
  ...
  if (parallel.boss) {
    ...
    g_out << " step: " << globals.step << std::scientific << std::setw(15)
          << vol << std::scientific << std::setw(15) << mass << std::scientific
          << std::setw(15) << mass / vol << std::scientific << std::setw(15)
          << press / vol << std::scientific << std::setw(15) << ie
          << std::scientific << std::setw(15) << ke << std::scientific
  ...
  if (globals.complete) {
    ...
    if (parallel.boss) {
      if (globals.config.test_problem >= 1) {
        if (globals.config.test_problem == 1)
          qa_diff = cl::sycl::fabs((100.0 * (ke / 1.82280367310258)) - 100.0);
        if (globals.config.test_problem == 2)
          qa_diff = cl::sycl::fabs((100.0 * (ke / 1.19316898756307)) - 100.0);
        if (globals.config.test_problem == 3)
          qa_diff = cl::sycl::fabs((100.0 * (ke / 2.58984003503994)) - 100.0);
        if (globals.config.test_problem == 4)
          qa_diff = cl::sycl::fabs((100.0 * (ke / 0.307475452287895)) - 100.0);
        if (globals.config.test_problem == 5)
          qa_diff = cl::sycl::fabs((100.0 * (ke / 4.85350315783719)) - 100.0);
        std::cout << "Test problem " << globals.config.test_problem
                  << " is within " << qa_diff << "% of the expected solution"
                  << std::endl;
        g_out << "Test problem " << globals.config.test_problem << " is within "
              << qa_diff << "% of the expected solution" << std::endl;
        if (qa_diff < 0.001) {
