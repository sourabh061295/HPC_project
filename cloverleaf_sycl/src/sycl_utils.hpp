...
// kernel calls #define SYNC_KERNELS // enable for fully synchronous (e.g
// queue.wait_and_throw()) kernel calls #define SYCL_FLIP_2D // enable for
// flipped id<2> indices from SYCL default
...
struct Accessor {
  typedef cl::sycl::accessor<T, N, mode,
                             cl::sycl::access::target::global_buffer>
      Type;
  typedef cl::sycl::accessor<T, N, mode, cl::sycl::access::target::host_buffer>
      HostType;

  inline static Type from(cl::sycl::buffer<T, N>& b, cl::sycl::handler& cgh) {
    return b.template get_access<mode, cl::sycl::access::target::global_buffer>(
        cgh);
  }
...
struct Buffer {
  ...
  inline typename Accessor<T, N, mode>::Type access(cl::sycl::handler& cgh) {
  ...
  template <cl::sycl::access::mode mode>
  inline typename Accessor<T, N, mode>::HostType access() {
...
static inline cl::sycl::id<2> offset(const cl::sycl::id<2> idx, const int j,
                                     const int k) {
...
static inline void par_ranged(cl::sycl::handler& cgh, const Range1d& range,
                              functorT functor) {
  cgh.parallel_for<nameT>(cl::sycl::range<1>(range.size),
                          cl::sycl::id<1>(range.from), functor);
}

// delegates to parallel_for, handles flipping if enabled
template <typename nameT, class functorT>
static inline void par_ranged(cl::sycl::handler& cgh, const Range2d& range,
                              functorT functor) {
#ifdef SYCL_FLIP_2D
  cgh.parallel_for<nameT>(
      cl::sycl::range<2>(range.sizeY, range.sizeX),
      cl::sycl::id<2>(range.fromY, range.fromX),
      [=](cl::sycl::id<2> idx) { functor(cl::sycl::id<2>(idx[1], idx[0])); });
#else
  cgh.parallel_for<nameT>(cl::sycl::range<2>(range.sizeX, range.sizeY),
